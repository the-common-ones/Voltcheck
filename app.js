...
// ── REGION DATA ───────────────────────────────────────────────────────────────
var RRATES={
  ph:{min:0.10,max:0.22,mid:0.16},   // Philippines - Meralco 2025
  co:{min:0.07,max:0.15,mid:0.11},   // Colombia - UPME 2025
  br:{min:0.13,max:0.20,mid:0.17},   // Brazil - ANEEL 2025
  dr:{min:0.12,max:0.18,mid:0.15},   // Dominican Republic - SIE 2025
  def:{min:0.08,max:0.20,mid:0.14}
};
var RSY={
  ph:{avg:1350,b20:1100,t20:1500},   // Philippine DOE
  co:{avg:1350,b20:975,t20:1750},    // Global Solar Atlas Colombia
  br:{avg:1650,b20:1400,t20:2000},   // ANEEL/EPE Brazil
  dr:{avg:1700,b20:1500,t20:1900},   // Global Solar Atlas DR
  def:{avg:1300,b20:1000,t20:1600}
};
var RPSH={
  ph:{min:4.5,max:5.5,mid:5.0},
  co:{min:4.5,max:5.5,mid:5.0},
  br:{min:4.5,max:6.5,mid:5.5},
  dr:{min:5.5,max:6.5,mid:6.0},
  def:{min:4.5,max:5.5,mid:5.0}
};
...
function degradedPR(yr){return Math.max(0.60,PR_BASE-Math.max(0,2025-parseInt(yr||2020))*0.005);}
...
function getRates(r){return RRATES[r]||RRATES.def;}
function getSY(r){return RSY[r]||RSY.def;}
function getPSH(r){return RPSH[r]||RPSH.def;}

function getRegionLabel(r){
  if(r==='ph')return'Southeast Asia - Philippines';
  if(r==='co')return'South America - Colombia';
  if(r==='br')return'South America - Brazil';
  if(r==='dr')return'Caribbean - Dominican Republic';
...
function getSourceNote(r,syB20,syT20,rMin,rMax,pMin,pMax){
  ...
  if(r==='co')return'Benchmarks: UPME Colombia, Global Solar Atlas Colombia, Intratec 2025. Rate range: $'+rMin+'-$'+rMax+'/kWh. '+base;
  if(r==='br')return'Benchmarks: ANEEL Brazil, EPE Brazil, Global Solar Atlas Brazil, Intratec 2025. Rate range: $'+rMin+'-$'+rMax+'/kWh. '+base;
  if(r==='dr')return'Benchmarks: SIE Dominican Republic, Global Solar Atlas DR, Climatescope 2025, IRENA DR Energy Profile 2024. Rate range: $'+rMin+'-$'+rMax+'/kWh. '+base;
  return'Benchmarks: Philippine DOE, Meralco rates, Global Solar Atlas Philippines. Rate range: $'+rMin+'-$'+rMax+'/kWh. '+base;
}
...
function updateCountryField(){
  ...
  if(r){
    ...
    if(r==='ph')i.value='Philippines';
    else if(r==='co')i.value='Colombia';
    else if(r==='br')i.value='Brazil';
    else if(r==='dr')i.value='Dominican Republic';
    else i.value='';
  } else {
...
function run(){
  ...
  var ageYrs=year?Math.max(0,2025-parseInt(year)):0;
